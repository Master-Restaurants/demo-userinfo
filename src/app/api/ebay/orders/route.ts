...
export async function GET(request: Request) {
  if (isDemoMode()) return NextResponse.json(demoOrdersPayload("ebay"));
  try {
    ...
    if (cached.state === "miss") {
      return NextResponse.json({
        ...
        totalCount: 0,
        items: [] as EbayOrderListRow[],
      });
    }

    const filtered = filterOrdersByCreatedRange(cached.value, startMs, endMs);

    const items: EbayOrderListRow[] = filtered.map((o) => ({
      orderId: o.id,
