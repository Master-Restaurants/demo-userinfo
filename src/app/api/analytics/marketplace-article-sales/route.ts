import { demoMarketplaceArticleSalesPayload } from "@/shared/lib/demoSeed";
...
export async function GET(request: Request) {
  if (isDemoMode()) return NextResponse.json(demoMarketplaceArticleSalesPayload());
  try {
    ...
    if (marketplace === "fressnapf" || marketplace === "fressnapf-at") {
      ...
      // Beide Slugs teilen sich denselben Mirakl-Operator-API
      // (fressnapfde-prod.mirakl.net hostet `fn_de` + `fn_at`). Trennung erfolgt
      // pro raw Order anhand `shipping_zone_code` ("DE"/"AT").
      const targetZone = marketplace === "fressnapf-at" ? "AT" : "DE";
      const currentLines: ReturnType<typeof extractLinesFromFlexRawOrder> = [];
      const previousLines: ReturnType<typeof extractLinesFromFlexRawOrder> = [];
      for (const order of raw) {
        const t = getCreatedMsFromFlexRaw(order);
        ...
        const zone =
          order && typeof order === "object"
            ? String(
                (order as Record<string, unknown>).shipping_zone_code ?? ""
              ).toUpperCase()
            : "";
