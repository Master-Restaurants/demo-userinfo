import { demoSalesPayload } from "@/shared/lib/demoSeed";
...
import {
  ...
  parseFressnapfCountryParam,
  parseYmdParam,
  summarizeFressnapfOrders,
...
async function writeSyncRecord(args: {
  fromYmd?: string;
  toYmd?: string;
  summary: ReturnType<typeof summarizeFressnapfOrders>["summary"];
  previousSummary?: ReturnType<typeof summarizeFressnapfOrders>["summary"];
  points: ReturnType<typeof summarizeFressnapfOrders>["points"];
  previousPoints?: ReturnType<typeof summarizeFressnapfOrders>["points"];
...
export async function GET(request: Request) {
  if (isDemoMode()) return NextResponse.json(demoSalesPayload("fressnapf"));
  try {
    const feePolicy = await getMarketplaceFeePolicy("fressnapf");
    ...
    if (Object.values(missing).some(Boolean)) {
      return NextResponse.json(
        {
          error: "Fressnapf API ist nicht vollständig konfiguriert (Basis-URL und API-Key).",
    ...
    const country = parseFressnapfCountryParam(searchParams.get("country"));
    ...
    if (fromYmd && toYmd) {
      ...
      const spanDays = Math.round((currentEndMs - currentStartMs) / FRESSNAPF_DAY_MS);
      ...
      currentStartMs = now - days * FRESSNAPF_DAY_MS;
      currentEndMs = now;
      if (compare) {
        prevStartMs = now - days * 2 * FRESSNAPF_DAY_MS;
        prevEndMs = now - days * FRESSNAPF_DAY_MS;
      }
    }

    const allOrdersRaw = await fetchFressnapfOrdersPaginated(
      config,
      compare
    ...
    const allOrders = country ? filterOrdersByCountry(allOrdersRaw, country) : allOrdersRaw;
    ...
    if (compare) {
      ...
      const current = summarizeFressnapfOrders(currentRaw);
      const previous = summarizeFressnapfOrders(previousRaw);
      ...
      const meta = {
        ...
        source: "fressnapf_orders" as const,
      ...
      // sync-Tabelle nur für die kombinierte Sicht schreiben — pro-Land-Splits
      // teilen sich (period_from, period_to) und würden sich gegenseitig
      // überschreiben.
      if (!country) {
    ...
    const current = summarizeFressnapfOrders(filtered);
    const meta = {
      days,
