...
export type AmazonOrdersViewProps = {
  /** API-Endpoint für Bestellungen (ohne Query). Default `/api/amazon/orders` — Country-Varianten z. B. `/api/amazon/amazon-fr/orders`. */
  apiUrl?: string;
  /** Cache-Key für localStorage. Pro Land eigener Key. */
  cacheKey?: string;
  /** Optional: Titel für Logo/H1 (z. B. "Amazon FR · Bestellungen"). */
  title?: string;
  /** Cache-Refresh-Marktplatz-Schlüssel (z. B. "amazon" oder "amazon-fr"). */
  cacheRefreshMarketplace?: string;
};

export function AmazonOrdersView({
  apiUrl = "/api/amazon/orders",
  cacheKey = "amazon_orders_accumulated_v1",
  title,
  cacheRefreshMarketplace = "amazon",
}: AmazonOrdersViewProps) {
  ...
  const [allRows, setAllRows] = useState<AmazonOrderRow[]>([]);
  ...
  const allRowsRef = useRef<AmazonOrderRow[]>([]);
  ...
  const summary = useMemo(() => {
    ...
    const fba = tableRows.filter((row) => row.fulfillment === "FBA").length;
    const fbm = tableRows.filter((row) => row.fulfillment === "FBM").length;
  ...
  const loadOrders = useCallback(
    async (nextFrom?: string, nextTo?: string, forceRefresh = false, silent = false) => {
      ...
      try {
        ...
        const res = await fetch(`${apiUrl}?${search.toString()}`, { cache: "no-store" });
  ...
  const effectiveTitle = title ?? t("nav.amazonOrders");
    ...
    <div className={DASHBOARD_PAGE_SHELL}>
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <span className={cn(DASHBOARD_PAGE_TITLE, "text-muted-foreground")}>{effectiveTitle}</span>
        </div>
      </div>
        ...
        <div className="ml-auto flex flex-wrap items-end justify-end gap-2">
          ...
          <div className="flex items-end pb-0.5">
            <MarketplaceCacheRefreshButton
              marketplace={cacheRefreshMarketplace}
              resource="orders"
              fromYmd={from}
              toYmd={to}
      ...
      {isLoading && tableRows.length === 0 && allRows.length === 0 ? (
        <div className="rounded-xl border border-border/50 bg-card/80 p-4 text-sm text-muted-foreground">
          {t("ordersShared.loading", { marketplace: t("nav.amazon") })}
        </div>
      ) : (
        <DataTable
          columns={columns}
