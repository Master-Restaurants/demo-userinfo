...
/**
 * Trennt einen Marktplatz-Anzeigenamen wie "Fressnapf DE" / "Amazon DE" /
 * "Fressnapf AT" in {base, country}. Wenn das letzte Wort exakt 2-3 Großbuchstaben
 * ist, gilt es als Länder-Suffix — sonst kein Country-Chip. So bleiben Namen
 * ohne Land-Suffix (Otto, Kaufland, ...) chip-frei und wir kriegen DE/AT/FR-
 * Discrimination ohne extra Konfiguration.
 */
function splitMarketplaceName(name: string): { base: string; country: string | null } {
  const lastSpace = name.lastIndexOf(" ");
  if (lastSpace === -1) return { base: name, country: null };
  const tail = name.slice(lastSpace + 1);
  if (/^[A-Z]{2,3}$/.test(tail)) {
    return { base: name.slice(0, lastSpace), country: tail };
  }
  return { base: name, country: null };
}
                  ...
                  <Fragment key={mp.slug}>
                    ...
                    >
                      <td className="px-6 py-5">
                        {(() => {
                          const { country } = splitMarketplaceName(mp.name);
                          return (
                            <div className="flex items-center gap-4">
                              ...
                              />
                              <div
                                className="relative flex h-12 w-44 shrink-0 items-center justify-center overflow-hidden rounded-md border bg-background"
                                title={mp.name}
                              >
                                {/*
                                  Inner-Box mit Aspect 1:5 (32×160) garantiert,
                                  dass alle Logos bis ~5:1 Seitenverhältnis
                                  height-bound rendern → einheitlich 32px hoch,
                                  egal ob square (Otto/Kaufland) oder wordmark
                                  (amazon/fressnapf/zooplus/...). Sehr breite
                                  Logos (MediaMarkt & Saturn ~6:1) rendern
                                  geringfügig kleiner — akzeptabel.
                                */}
                                ...
                                {country ? (
                                  <span className="absolute right-1 top-1 rounded bg-foreground/90 px-1.5 py-0.5 text-[10px] font-bold uppercase leading-none text-background shadow-sm">
                                    {country}
                                  </span>
                                ) : null}
                              </div>
