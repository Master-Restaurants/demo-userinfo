...
import {
  ZOOPLUS_DAY_MS,
  fetchZooplusOrdersPaginated,
  filterOrdersByCreatedRange,
  getZooplusIntegrationConfig,
  parseYmdParam,
  summarizeZooplusOrders,
...
async function writeSyncRecord(args: {
  fromYmd?: string;
  toYmd?: string;
  summary: ReturnType<typeof summarizeZooplusOrders>["summary"];
  previousSummary?: ReturnType<typeof summarizeZooplusOrders>["summary"];
  points: ReturnType<typeof summarizeZooplusOrders>["points"];
  previousPoints?: ReturnType<typeof summarizeZooplusOrders>["points"];
...
export async function GET(request: Request) {
  if (isDemoMode()) return NextResponse.json(demoSalesPayload("zooplus"));
  try {
    const feePolicy = await getMarketplaceFeePolicy("zooplus");
    ...
    if (fromYmd && toYmd) {
      ...
      const spanDays = Math.round((currentEndMs - currentStartMs) / ZOOPLUS_DAY_MS);
      ...
      currentStartMs = now - days * ZOOPLUS_DAY_MS;
      currentEndMs = now;
      if (compare) {
        prevStartMs = now - days * 2 * ZOOPLUS_DAY_MS;
        prevEndMs = now - days * ZOOPLUS_DAY_MS;
      }
    }

    const allOrders = await fetchZooplusOrdersPaginated(
      config,
      compare
    ...
    if (compare) {
      ...
      const current = summarizeZooplusOrders(currentRaw);
      const previous = summarizeZooplusOrders(previousRaw);
      ...
      const meta = {
        ...
        source: "zooplus_orders" as const,
    ...
    const current = summarizeZooplusOrders(filtered);
    const meta = {
      days,
