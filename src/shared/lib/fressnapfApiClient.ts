...
export type FressnapfCountry = "DE" | "AT";
...
export type FressnapfNormalizedOrder = {
  ...
  status: string;
  /**
   * DE/AT-Kanal-Diskriminator. Aus `shipping_zone_code` ("DE"/"AT") oder als
   * Fallback aus `shipping_country_iso_code` (DEU→DE, AUT→AT). `null` bei
   * unbekannter Zone (z. B. ältere Orders ohne Zone). Der DE-Mirakl-Operator
   * `fressnapfde-prod.mirakl.net` bedient beide Channels (`fn_de` + `fn_at`);
   * splitten passiert in-memory nach Fetch.
   */
  country: FressnapfCountry | null;
};
...
function deriveCountry(o: Record<string, unknown>): FressnapfCountry | null {
  const zone = typeof o.shipping_zone_code === "string" ? o.shipping_zone_code.toUpperCase() : "";
  if (zone === "DE" || zone === "AT") return zone;
  const customer = (o.customer && typeof o.customer === "object" ? o.customer : {}) as Record<
    string,
    unknown
  >;
  const shipping = (
    typeof customer.shipping_address === "object" && customer.shipping_address
  ...
  const iso =
    typeof shipping.country_iso_code === "string" ? shipping.country_iso_code.toUpperCase() : "";
  if (iso === "DEU" || iso === "DE") return "DE";
  if (iso === "AUT" || iso === "AT") return "AT";
  return null;
}
...
export function normalizeFressnapfOrder(
  ...
  return {
    ...
    units,
    status,
    country: deriveCountry(o),
  };
}

export function filterOrdersByCountry(
  orders: FressnapfNormalizedOrder[],
  country: FressnapfCountry
): FressnapfNormalizedOrder[] {
  return orders.filter((o) => o.country === country);
}

export function parseFressnapfCountryParam(raw: string | null): FressnapfCountry | null {
  if (!raw) return null;
  const v = raw.trim().toLowerCase();
  if (v === "de" || v === "deu") return "DE";
  if (v === "at" || v === "aut") return "AT";
  return null;
}
