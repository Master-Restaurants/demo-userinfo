...
const REPORT_MARKETPLACES: ReportMarketplaceConfig[] = [
  ...
  },
  {
    ...
    salesEndpoint: "/api/fressnapf/sales?country=de",
  ...
  },
  {
    ...
    name: "Fressnapf AT",
    logo: "/brand/marketplaces/fressnapf.svg",
    salesEndpoint: "/api/fressnapf/sales?country=at",
    payoutSlug: "fressnapf",
  },
  {
...
async function fetchMarketplaceSales(args: {
  ...
  // Endpoint kann bereits Query-String enthalten (z. B. `?country=at` für
  // Fressnapf-AT). URL-API-Komposition statt Konkatenation.
  const u = new URL(args.endpoint, args.origin);
  u.searchParams.set("from", args.fromCurrent);
  u.searchParams.set("to", args.toCurrent);
  u.searchParams.set("compare", "1");
  u.searchParams.set("compareMode", "previous");
...
async function fetchTopSkus(args: {
  ...
  if (previousPayouts.length === 0 && currentPayouts.length > 0) {
    ...
    if (olderPayouts.length > 0) {
      // Nur EIN älterer Payout — sonst landen bei Amazon's bi-wöchentlichen
      // 14-Tage-Settlements ZWEI ältere im previous-Bucket (Edge-Case
      // period_to == latestOlderToMs - 14d wurde vorher mit `>=` mit-
      // eingeschlossen). Effekt: previous-Revenue ~verdoppelt, jede SKU
      // erscheint als Verlierer, Top-Gewinner verschwinden strukturell.
      previousPayouts = [olderPayouts[0]];
    }
  }
