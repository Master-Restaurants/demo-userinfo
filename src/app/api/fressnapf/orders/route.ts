import { demoOrdersPayload } from "@/shared/lib/demoSeed";
...
export async function GET(request: Request) {
  if (isDemoMode()) return NextResponse.json(demoOrdersPayload("fressnapf"));
  try {
    ...
    const filteredByDate = filterOrdersByCreatedRange(cached.value, startMs, endMs);
    const filtered = country ? filterOrdersByCountry(filteredByDate, country) : filteredByDate;
